        ...
        with st.container():
            ...
            with col2:
                if st.button(
                    ...
                    st.session_state.current_block_index = 0  # Initialize to first section
                    st.session_state.scroll_to_top = True
                    # Update URL with lesson info including block_index
        ...
        with col_prev:
            if current_idx > 0:
                if st.button("⬅️ Prev", use_container_width=True):
                    ...
                    st.session_state.scroll_to_top = True
                    # Update URL with new block_index
                    st.query_params.update({
                        "page": "lesson",
                        "lesson_id": str(lesson.lesson_id),
        ...
        with col_next:
            if current_idx < total_blocks - 1:
                ...
                if st.button("Quiz ➡️", use_container_width=True):
                    st.session_state.current_block_index += 1
                    st.session_state.scroll_to_top = True
                    # Update URL with new block_index (quiz section)
                    st.query_params.update({
                        "page": "lesson",
                        "lesson_id": str(lesson.lesson_id),
                        "block_index": str(st.session_state.current_block_index)
